...
int nii_call_class_static_method(zval *object, char *class_name, char *method_name, zval **retval_ptr_ptr, zend_uint param_count, zval **params[]) {
	...
	NII_NEW_STRING(class_name_zv, class_name);
	add_next_index_zval(fn, class_name_zv);
	NII_NEW_STRING(method_name_zv, method_name);
	...
	NII_PTR_DTOR(method_name_zv);
	NII_PTR_DTOR(class_name_zv);
	NII_PTR_DTOR(fn);
	return status;
}
...
int nii_class_has_constructor(zval *object TSRMLS_DC){
...
int nii_new_class_instance_0(zval **return_value, char *class_name TSRMLS_DC) {
	return nii_new_class_instance(return_value, class_name, 0, NULL TSRMLS_CC);
}
